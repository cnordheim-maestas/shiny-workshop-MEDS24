...
server <- function(input, output) {
  ...
  body_mass_df <- reactive({
    penguins %>%
      ## ask Sam to review this line of code:
  ...
  output$bodyMass_scatterplot_output <- renderPlot({
    ...
    # Sam says not to filter data in here
    
    ggplot(na.omit(body_mass_df()), # NOTE this is the df we built above
  ...
  years_df <-reactive({
    ...
    penguins %>%
      filter(year %in% c(input$year_input)) # referencing back to the year_input choices above
